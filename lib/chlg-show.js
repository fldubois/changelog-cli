...
function chlgShow(options, callback) {
  ...
  if (releases.length === 0) {
    ...
    for (var i = 0; i < releases.length; i++) {
      if (releases[i] !== 'all' && releases[i] !== 'latest' && releases[i] !== common.releases.UNRELEASED) {
        if (semver.valid(releases[i]) === null && semver.validRange(releases[i]) === null) {
          if (semver.valid(releases[i]) === null) {
            return callback(new Error('‘' + releases[i] + '’ is not valid semver version'));
          } else {
            return callback(new Error('‘' + releases[i] + '’ is not valid semver range'));
          }
        }
      }
    }
  ...
  var capture = {
    release: (releases.length === 1 && (releases[0] === 'all' || releases[0] === 'latest')) ? 'all' : false,
  ...
  stream.on('readable', function () {
    ...
    if (releases.length === 1 && releases[0] === 'latest') {
