...
function chlgRelease(release, options, callback) {
  ...
  chlgShow({file: file, releases: ['all']}, function (error, logs) {
    ...
    if (releases.length > 1) {
      ...
      } else if (logs[latest].date.getTime() >= now.getTime()) {
        return callback(new Error('last release date (' + dates.format(logs[latest].date) + ') is in the future'));
      }
    }

    var insert = '\n' + formatter.release(release, now);
